...
# rbind
sdf <- rbind(ptc %>% rename(survival_days = days) %>% mutate(group = "ptc") %>% select(-ptc),
             lp_no_ptc %>% select(survival_days, group),
             lp_ptc %>% rename(survival_days = total_surival) %>% mutate(group = "lp_ptc") %>% select(survival_days, group)) %>% 
...
gg <- ggsurvplot(fit, data = sdf, conf.int = F, xlab = "Time (Days)", pval = FALSE,
                 ...
                 palette = as.character(pal[c("blue", "grey", "red")])
)+
...
# import
sig_genes <- readRDS(glue("{repo_dir}outs/sig_genes_tam_gcis_separate_no_recurrence_filter.rds"))
...
tamp_samples <- metadata %>% filter(tam_status == "TAM+") %>% pull(mouse_id) %>% unique() %>% paste0(.,"_PT")
tamn_samples <- metadata %>% filter(tam_status == "TAM-") %>% pull(mouse_id) %>% unique() %>% paste0(.,"_PT")
...
op <- oncoPrint(mtx_list,
                ...
                row_names_gp = gpar(fontsize = 8, col = ifelse(rownames(mtx_list[[1]]) %in% c("Kcnb1", "Kcnh2"), "red", "black")),
                pct_gp = gpar(fontsize = 8),
                heatmap_legend_param = list(title = NULL, labels = c("TAM+", "TAM-"))
)
pdf(glue("{output_dir}/gcis_oncoprint.pdf"), h = 10, w = 8)
draw(op)
dev.off()
...
lp_volc <- res_filt %>%  
  ...
  mutate(colour = ifelse(padj > p_adj_cutoff | abs(log2FoldChange) < lfc_cutoff, "not_sig",
         ...
         label = ifelse(symbol %in% (filter(., colour != "not_sig") %>% slice_min(order_by = padj, n = 10) %>% pull(symbol)) | (grepl("Kcn", symbol) & colour == "up"),symbol,""))

# Set plot boundaries
xbound <- max(abs(lp_volc$log2FoldChange)) + 0.25
...
print(ggplot(lp_volc, aes(log2FoldChange, -log10(padj), label = label, colour = colour))+
        geom_point()+
        geom_text_repel(size = ifelse(grepl("Kcn", lp_volc$symbol), 3.5 ,3.5),
                        segment.size = 0.2, box.padding = 0.5,
                        color = ifelse(grepl("Kcn", lp_volc$symbol), "red" ,"black"),
