...
proj_dir <- "/Users/anders/Library/CloudStorage/OneDrive-UniversityofToronto/PhD/Projects/Sleeping_Beauty/Lazy_Piggy/"
repo_dir <- "/Users/anders/Library/CloudStorage/OneDrive-UniversityofToronto/PhD/repos/lazy_piggy/"
setwd(glue("{dirname(proj_dir)}/lazy_piggy_repo"))

# Palette -----------------------------------------------------------------
pal <- c(blue = "#244583", purple = "#902A7A", red = "#C8222A", yellow = "#F1E902", green = "#1D9849", orange = "#F68B1F", grey = "#ADAFB2")
...
gg <- ggsurvplot(fit, data = sdf, conf.int = F, xlab = "Time (Days)", pval = FALSE,
                 ...
                 legend.labs = c("Nestin:Cre+/-; N-Luc-SB100+/-; R26-PB-ER+/-; LP+/-", "Nestin:Cre+/-; N-Luc-SB100+/-; R26-PB-ER+/-; LP+/-; Ptc+/-", "Ptc+/-"),
                 censor.shape = c("|"), censor.size = 6, 
                 axes.offset = T, tables.y.text = FALSE,
                 palette = as.character(pal[c("red", "blue", "grey")])
)+
...
# Panel B -----------------------------------------------------------------
## inputs
input_file <- glue::glue("{dirname(proj_dir)}/Lazy Piggy/20211201/all_annotations_20211201.annot") # tsv of annotated insertions merged from all samples
